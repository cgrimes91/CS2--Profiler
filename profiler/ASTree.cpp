...
//
void AST::swap(AST& b) {
  nodes nodetmp= nodeType;
  nodeType=b.nodeType;
  b.nodeType=nodetmp;
  std::string stringtmp=tag;
  tag=b.tag;
  b.tag=stringtmp;
  std::string stringtmp_close=closeTag;
  closeTag=b.closeTag;
  b.closeTag=stringtmp_close;
  child.swap(b.child);
  std::string txtTmp= text;
  text=b.text;
  b.text=txtTmp;
}
...
void AST::mainHeader(const std::vector<std::string>& profileName) {
  ...
  while((((*i)->tag)!="function")&&(i!=child.end())){
    ++i;
  }
  std::vector<std::string> newVec=profileName;
  std::string astString="#include \"profile.hpp\"\n";
  child.insert(i,new AST(token,astString));
  std::vector<std::string>::iterator j= newVec.begin();
  std::vector<std::string>::const_iterator k= profileName.begin();
  while(j!=newVec.end()){
    size_t tmp=(j)->find('_');
    std::string fileName=(j)->replace(tmp,1,".");
    std::string objectName="profile "+*k+"(\""+*j+"\");"+"\n";
    child.insert(i,new AST(token, objectName));
    ++j;
    ++k;
  }
...
void AST::fileHeader(const std::string& profileName) {
  ...
  std::string astString="#include \"profile.hpp\"\n";
  child.insert(i,new AST(token,astString));
  std::string externName="extern profile "+profileName+";"+"\n\n";
  child.insert(i,new AST(token, externName)); 
}
...
void AST::mainReport(const std::vector<std::string>& profileName) {
  ...
  std::list<AST*>::iterator mainFunc;
  std::list<AST*>::iterator k= child.begin();
  bool mainFound=0;
  while((k!=child.end())&&(!mainFound)){
    while((((*k)->tag)!="function")&&(k!=child.end())){
    ++k;
    }
    if((*k)->tag=="function"){
      std::list<AST*>::iterator j= (*k)->child.begin();
      bool notFound=0;
      while((j!=(*k)->child.end())&&(!notFound)&&(!mainFound)){
        while((((*j)->tag)!="name")&&(j!=(*k)->child.end()))
          ++j;
        if((*j)->getName()=="main"){
          mainFunc=k;
          mainFound=1;
  ...
  std::list<AST*>::iterator n=(*mainFunc)->child.begin();
   while((((*n)->tag)!="block")&&(n!=(*mainFunc)->child.end()))
     ++n;
  std::list<AST*>::iterator z=(*n)->child.begin();
   while((((*z)->tag)!="return")&&(z!=(*n)->child.end()))
     ++z;
  std::vector<std::string> myString= profileName;
  for(size_t i=0; i< myString.size(); ++i){
    std::string prntOut= "\nstd::cout<< "+ myString[i] +"<< std::endl;\n";
    child.insert(z, new AST(token, prntOut));
  }
}
...
void AST::funcCount(const std::string& profileName) {
    ...
    //
  std::list<AST*>::iterator i= child.begin();
  std::list<AST*>::iterator q;
  while (i!=child.end()){
    if(((*i)->tag=="function")||((*i)->tag=="constructor")||((*i)->tag=="destructor")){
      std::list<AST*>::iterator j=(*i)->child.begin();
      while(j!=(*i)->child.end()){
        if((*j)->tag=="name"){
	  std::list<AST*>::iterator z=(*i)->child.begin();
          while(z!=(*i)->child.end()){
            if((*z)->tag=="block"){
	      std::list<AST*>::iterator y= (*z)->child.begin();
              while(y!=(*z)->child.end()){
                if((*y)->text=="{")
                  q=y;
                ++y;
	    ...
	    ++z;
          ...
          std::string funcName=(*j)->getName();
	  std::string toInsert=" "+ profileName+".count(__LINE__, \""+funcName+"\");";
          child.insert(++q, new AST(token, toInsert));
        }
        ++j;
      }
    }
...
void AST::lineCount(const std::string& profileName) {
  ...
  std::string toInsert= " "+profileName+".count(__LINE__);";
  std::list<AST*>::iterator i= child.begin();
  while(i!= child.end()){
    if(!(*i)->child.empty()){
    ...
    if((*i)->tag=="expr_stmt"){
      if(!isStopTag((*i)->tag))
        (*i)->child.push_back(new AST(token, toInsert));
    }
    ++i;
  }        
}
